...
export default function App() {
  ...
  const [command,setCommand]=useState<{type:'start'|'jump'|'reset';nonce:number}|null>(null);
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>panelRef.current?.querySelector<HTMLButtonElement>('button')?.focus(),30);
    const keydown=(e:KeyboardEvent)=>{
      if(e.key==='Escape'){setActive(null);setHelp(false);}
      if(e.key==='Tab'){
  ...
  const sendCommand=(type:'start'|'jump'|'reset')=>setCommand({type,nonce:Date.now()});
      ...
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className={`world-area ${mode==='read'?'reading-mode':''}`}>
          ...
          {mode==='play'?<>
            ...
            <Suspense fallback={<div className="world"><div className="world-loading">Growing a little world…</div></div>}><World night={night} sound={sound} paused={!!active||help} reducedMotion={reducedMotion} onVisit={visit} onCollect={index=>setSparksFound(previous=>previous.includes(index)?previous:[...previous,index])} collected={sparksFound} visited={visited} command={command}/></Suspense>
